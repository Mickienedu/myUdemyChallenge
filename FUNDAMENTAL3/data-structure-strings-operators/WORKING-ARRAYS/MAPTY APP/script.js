...
class App {
  #map;
  #mapZoomLevel = 13;  // 241. Move to Marker On Click 
  #mapEvent;
  #workouts = [];
  ...
  constructor() {
    ...
    inputType.addEventListener("change", this._toggleElevationField);
    containerWorkouts.addEventListener('click', this._moveToPopup.bind(this)); // 241. Move to Marker On Click 
  ...
  _loadMap(position) {
    ...
    this.#map = L.map("map").setView(coords, this.#mapZoomLevel); //added 'coords' for perfect location
  ...
  _newWorkout(e) {
    ...
    // set local storage to all workout| 242. Working with localStorage
    this._setLocalStorage();
  }
  _renderWorkoutMarker(workout) {
     // 234. Displaying a Map Marker
     L.marker(workout.coords)
       .addTo(this.#map) //added 'coords'
       .bindPopup(
         L.popup({
           maxWidth: 250,
           minWidth: 100,
...
/* local storage is a place in the browser where we can store data
that will stay there even after we close the page */
